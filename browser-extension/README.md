...
The task ID and local endpoint live in the URL fragment and tab `sessionStorage`; Hyatt does not require them. The content script cannot call the local API directly: it sends a private extension message to `background.js`, and that service worker accepts requests only from Hyatt task tabs and local HTTP endpoints. The popup only retries the task already associated with the active tab. It does not accept an arbitrary booking ID or parse prices independently.
...
The server-side planner and content script execute the same shared rule module in `safetyRules.js`. Task fragment and session-storage keys come from the shared `taskProtocol.js`. The extension fails closed if either module is unavailable. Browser-task routes reject Hyatt/page origins and return CORS only to a `chrome-extension://` origin. Set `TRIPBUDDY_BROWSER_EXTENSION_ORIGIN` to the origin shown for the unpacked extension (for example `chrome-extension://<32-letter-id>`) to pin the server to that exact installation; when omitted, any syntactically valid Chrome extension origin is accepted, but ordinary page scripts are still rejected.
