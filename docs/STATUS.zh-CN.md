...
- **同一调用一回合内只跑一次**;工具目录按代价(免费 / 需要按下)分组呈现给模型;`tools` 步骤的说明只在确认按钮之前出现,自由的读取不再逐步解说。
...
| **(未提交工作树)Agent loop 重构后** | 68 文件 514 项通过;typecheck、lint 均无告警;build 成功,`/` 与 `/desk` 为 dynamic(`ƒ`) | `npm test` / `npm run typecheck` / `npm run lint` / `npm run build` |
