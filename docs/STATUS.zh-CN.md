...
- **一个回合只有一个出口**:所有路径返回一个结果对象,只有一处 emit 终止事件。「失败不丢已完成的工作」「RUN_ERROR 之后没有 RUN_FINISHED」两条不变量只写在那一处,并有测试遍历回合能结束的每一种形状。
- **跨回合记忆统一为一份**:回合结束时留下已展示的行引用与已产生的搜索;客户端原样存取、不读不构造。上一轮的行引用因此在下一轮仍然有效,不再需要模型重新列一遍。
- **完全相同的调用一回合内只跑一次**(参数不同是不同的问题,不去重);单个能力一回合最多 4 次;一个回合最多 12 步;工具目录按代价(免费 / 需要按下)分组呈现给模型;`tools` 步骤的说明只在确认按钮之前出现,自由的读取不再逐步解说。
- **回合中途失败不丢已完成的工作**:一次浏览器抓取是最贵的操作(一次点击、一次等待、一个真实标签页)。回合后半段出错时,已拿到并渲染的结果保留,并明说「不用重新查一遍」;只有一无所获时才报错。provider 的瞬时空响应也已并入可重试。
...
- **浏览器任务直接执行,不再要求额外按一下**([ADR 0007](./decisions/0007-agent-opens-the-tab.md)):开口要求就是发起,标签页仍在用户自己的 Chrome 里、仍然可见。标签页由发送消息这个手势打开(Chrome 只允许在手势内 `window.open`);消息看起来不需要浏览器时不预开,猜错了就在对话里给一个链接——服务端已经在等这个任务,任务过期前点开都能完成同一次运行。
- **只有写操作仍然要按一下**,并由产品文案写清这次按下会改什么。不对称的理由是猜错的代价不同:读错了再读一遍,写错了已经发生。
- 回合会跨越浏览器等待:Companion 回传的证据进入同一次对话,而不是另开一个页面。**标签页 25 秒内没有接手就报「从未打开」**,不再干等满 3 分钟——「页面没打开」和「页面不肯给价」对用户是不同的事。
