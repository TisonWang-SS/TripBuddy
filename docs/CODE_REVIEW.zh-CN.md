...
### 3.30 就搜索结果追问,产品又去重搜了一遍 — ✅ 已完成(本次修复)
...
**这条和 §3.28、§3.29 是同一类**:能力早就写好了,断的是**让模型知道它可用**的那条线。§3.29 断在验证语义,这条断在上下文传递。**一个 agent 产品里,「工具存在」和「工具够得着」是两件需要分别验证的事。**
...
| 38 | 补记 `99bc9cb` 的 OTA 价格源:ADR 0006 + PRD 边界 + STATUS;全包价假设改为具名常量并钉上测试 | 本次修复 |
| 38 | 单轮意图路由改写为多轮 Agent loop,对话成为产品主界面(ADR 0005);六个缺陷见 §3.28 | 本次修复 |
| 39 | 日期 grounding 从「等于抽取器结果」改为「属于候选集」,失败转为追问(§3.29) | 本次修复 |
| 40 | 本次对话已收集的搜索随回合回传,模型可复用而不重搜;新鲜度 15 分钟明示(§3.30) | 本次修复 |
| 40 | 补 `set_search_budget` 让追加条件复用已有结果;能力 `precheck` 把币种冲突提到确认之前(§3.30) | 本次修复 |
