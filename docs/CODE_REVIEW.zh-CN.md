...
### 3.37 确认按钮每次都出现、每次都被按下,那它就不是同意 — ✅ 已完成(本次变更)

产品判断:「一次按下只授权一次,这个授权取消,不然 human in the loop 的环节太多了。」

**这一条推翻了写下来的边界**,所以先说清推翻的是什么:`PRD.md` 要求每次查价「explicitly initiated by the user」,ADR 0001 说队列项「starts after the user clicks」。真正被取消的不是「明示发起」,而是**对一个已经做出的决定再确认一次**——用户打出「查一下上海 9 月 1 日的酒店」就是明示发起,标签页仍然可见、仍在他自己的 Chrome 里。写成 [ADR 0007](./decisions/0007-agent-opens-the-tab.md),并把 PRD 与 ADR 0001 的措辞收窄到这个意思。

**保留写操作的确认。** 不对称的理由是猜错的代价不同:读错了再读一遍,写错了已经发生、要用户自己回退。`requiresConfirmation` 现在是 `effect === "write"`,`invokeCapability` 那道最后的闸门照旧。

**标签页改由发送消息这个手势打开。** Chrome 只允许在手势内 `window.open`,而确认按钮消失后,按下发送是这一轮唯一的手势。客户端在消息看起来需要浏览器时预开一个空标签,运行拿到地址后再导航过去。

这个猜测**允许猜错,两个方向都允许**:多开一个标签,用完关掉;没开成(猜错或被拦),launch 到达时没有标签接,对话里给一个链接——服务端已经在等这个任务,任务过期前点开都能完成同一次运行。两种都不是正确性问题,这才是猜测可以停留在关键词匹配、而不必去预测 planner 的原因。

**取消确认顺带打断了两条挂在确认分支上的逻辑,都是真回归:**

1. **`precheck` 停跑了。** 它写在 needsPress 分支里,浏览器任务不再走那条路,币种检查就跟着不跑了——**一个币种不符的预算会直接到达 Hyatt**。现在每个调用之前都跑,与 effect 无关。
2. **搜索前的说明没了。** 同一个原因。现在改成:凡是有代价的动作(开标签、改设置)之前都说;自由的读取仍然不说,它有自己的进度行和结果卡。
3. 另外把 `parseCapabilityArgs` 移出确认分支时,它落到了捕获 `CapabilityArgsError` 的 try 之外——参数错误会变成 RUN_ERROR 而不是追问。已放回。

**再补一处这次才暴露的健壮性缺口:标签页没打开时,服务端只会干等满 3 分钟 TTL。** 有确认按钮时这不可能发生(按下即打开);现在标签页是猜出来的,「没人接这个任务」成了真实状态。Companion 会在第一张快照时把任务从 `pending` 推到 `running`,所以**25 秒后仍是 `pending` 就说明没有标签在处理它**——报 `never_started` 而不是等到超时。两者对用户是不同的事:一个是「页面没打开」(点链接就能继续同一次运行),一个是「页面不肯给出价格」。

**实测**(真实浏览器 + 真实模型):

| | 结果 |
|---|---|
| 「查一下…上海住1晚的酒店」 | `search_hotels` **直接执行**,无确认卡;`window.open` 在按下发送的那一刻被调用 |
| 「我有哪些预订？」 | `window.open` **调用 0 次**,读取路径不预开标签 |
| 「帮我盯着我那个吉隆坡的预订」 | 仍然出确认卡,文案写清每 72 小时查一次 |
| curl 环境(没有浏览器)的 5 条搜索 | 26 秒内报 `never_started`,消息可操作;其余 17 条正常 |

**顺带按产品要求调整了预算**:一个回合 6 步 → 12 步(现在一个回合能跑完 搜索 → 验含税价 → 套预算 → 与现有预订比,不必让用户中途再开口);单个能力一回合最多 4 次;**完全相同的调用(能力 + 参数逐字相同)仍然只跑一次**——参数不同是不同的问题,不去重。

### 3.38 28/28 全绿的那份记录里,有两处产品在自相矛盾 — ✅ 已完成(本次修复)

用户把评测跑在能连真实浏览器的环境里,产出完整 trace(`data/evals/agent-loop/`)。**摘要是 28/28 clean、0 miss。** 读记录,两处明确的缺陷:

**其一:什么都没跑,却说「上面的结果是准确的」。**

```
> 我要去上海出差[大段重复]…
< 我没能写出一段自己信得过的总结——它反复给出上面材料里没有的数字。
  上面的结果本身是准确的,你可以直接看…
```

那一轮 `tools=[]`——**一个工具都没调,上面什么都没有**。这句兜底文案(§3.34 加的)默认「失败发生在有结果之后」,而它同样会在一无所获时触发。比不说话更糟:它让人去找一个不存在的东西。现在两条兜底都按有没有结果分岔。

**其二:先宣告要做,再说做不了。**

```
> 查一下2020年1月1日上海的酒店
< 正在搜索上海2020年1月1日入住的Hyatt酒店。  入住日期已经过去,请告诉我一个今天或之后的入住日期。
```

`gap-dates` 同样:「我先按明天入住一晚来搜索。 我没有把握读准这次入住的日期。」——模型描述了一个**不会发生的动作**。

根因是 §3.34 的 `withNote`:参数校验失败时保留模型的说明。当时是对的——「您提到的是希尔顿,但本产品只收集凯悦」在被拒之后仍然成立。但模型写的 note 混了两类内容:**关于请求的事实**(拒绝后仍成立)和**关于动作的宣告**(拒绝后就是假的)。

机械区分不了,所以改提示词的定位:message 说的是**用户的请求需要知道什么**,不是**我即将做什么**——进度行和结果卡已经在说后者了。修完:「入住日期已经过去,无法搜索。请提供未来的日期。」

**两条都补进了场景集**(`corner-no-results-fallback`、`corner-refused-announcement`),并给断言加了 `saysNot`——有些缺陷的形状是「不该说某句话」。

**方法学**:这份 trace 最有价值的部分不是那个 28/28,是它下面的文字。**28/28 是机械断言全过,而机械断言只覆盖「调了哪个工具、有没有要按下、有没有开标签」**——两处矛盾都发生在这些维度全对的前提下。这与 §3.34 的结论是同一条,只是这次连「跑完全绿」都不足以说明问题:**评测的产出是记录,不是分数**。
...
| 47 | Agent loop 重构:单一出口、`TurnMemory` 统一跨回合状态、只在按下前说明、同一调用一回合只跑一次;补 `get_hotel_offer_detail` 与按代价分组的目录(§3.36) | `62c0076` |
| 48 | 浏览器任务取消确认(ADR 0007),标签页改由发送手势打开;修 `precheck`/说明/参数捕获三处回归;补「标签页从未打开」的判定;步数上限上调(§3.37) | `ee4ca13` |
| 49 | 场景集入库并接进 CI 与文档验证规则;修 trace 里读出的两处自相矛盾(§3.38) | 本次修复 |
