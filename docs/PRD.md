...
- Future Hyatt data-extraction changes should preserve the real Chrome TripBuddy profile plus Browser Companion extension approach unless a tested replacement proves more reliable. CDP automation should not be treated as the primary Hyatt path without fresh real-world verification.
...
- Any behavior-changing update to browser evidence extraction must include at least one real Hyatt page test through the TripBuddy Chrome profile and Browser Companion extension, in addition to unit tests and build checks.
