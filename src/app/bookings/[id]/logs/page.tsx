...
import { Badge, LabelBadge } from "@/ui";
          ...
          <div className="list">{booking.observations.map((observation) => {
            ...
            return <article className="listItem evidenceItem" key={observation.id}>
              <div>
                ...
                <p>{observation.sourceName} · {sourceTypeLabel(observation.sourceType).label} · {collectionMethodLabel(observation.collectionMethod).label}</p>
                <p className="muted">Extractor: {observation.extractorName} v{observation.extractorVersion} · {extractionSourceLabel(observation.extractionSource).label}</p>
                <p>Room: {observation.roomTypeRaw ?? "Not captured"} · Policy: {observation.cancellationPolicyRaw ?? "Not captured"}</p>
                <p className="muted">{roomMatchLabel(observation.evidence?.roomMatch).label} room · {cancellationMatchLabel(observation.evidence?.cancellationMatch).label} cancellation · Taxes: {inclusionLabel(observation.evidence?.taxesIncluded).label} · Fees: {inclusionLabel(observation.evidence?.feesIncluded).label}</p>
