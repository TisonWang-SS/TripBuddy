...
export async function routeIntent(message: string, options: RouteOptions = {}): Promise<RouteDecision> {
  ...
  try {
    ...
    return decide(parseRouterOutput(payload), "model");
  } catch (error) {
    /*
...
export function buildRouterInstructions() {
  ...
  return [
    ...
    'Dates must be calendar dates formatted "YYYY-MM-DD". If the request gives a relative date such as "next week", omit the parameter instead of computing one.',
    "",
