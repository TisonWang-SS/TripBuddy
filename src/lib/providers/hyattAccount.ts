import type {
  AccountBookingExtraction,
  AccountPageSnapshot,
  ExtractedAccountBooking
} from "@/lib/providers/types";
import { HYATT_CURRENCY_PATTERN, normalizeHyattCurrency } from "@/lib/providers/hyattCurrency";

export type { AccountBookingExtraction, AccountPageSnapshot, ExtractedAccountBooking } from "@/lib/providers/types";
...
export function parseHyattAccountBookingsFromSnapshots(snapshots: AccountPageSnapshot[]): AccountBookingExtraction {
  ...
  const bookings = mergeBookingDetails(
    snapshots.flatMap((snapshot) =>
      extractHyattBookingSegments(snapshot.text)
        .map((segment) => parseHyattBookingSegment(segment, isHyattReservationDetailUrl(snapshot.url) ? snapshot.url : null))
        .filter(isBooking)
    )
  );
...
export function isHyattReservationDetailUrl(value: string) {
  try {
    const url = new URL(value);
    return /(^|\.)hyatt\.com$/i.test(url.hostname) &&
      /\/(?:res(?:ervation)?|reservations?|detail|stay-details)(?:\/|\b)/i.test(url.pathname);
  } catch {
    return false;
  }
}
...
function parseHyattBookingSegment(segment: string, bookingUrl: string | null): ExtractedAccountBooking | null {
...
function extractTotal(segment: string) {
  const totalPattern = new RegExp(`(?:Grand Total|Stay Total|Total Cash|Total for Stay|Total Cost Per Room\\*?|Total)\\s*(${HYATT_CURRENCY_PATTERN})\\s*([0-9][0-9,]*(?:\\.\\d{2})?)`, "i");
  const totalMatch = segment.match(totalPattern);
  if (totalMatch) {
    return { amount: parseAmount(totalMatch[2]), currency: normalizeHyattCurrency(totalMatch[1]) };
  }

  const reversedPattern = new RegExp(`(${HYATT_CURRENCY_PATTERN})\\s*([0-9][0-9,]*(?:\\.\\d{2})?)\\s*(?:Grand Total|Stay Total|Total Cash|Total for Stay|Total Cost Per Room\\*?|Total)`, "i");
  const reversedMatch = segment.match(reversedPattern);
  return reversedMatch ? { amount: parseAmount(reversedMatch[2]), currency: normalizeHyattCurrency(reversedMatch[1]) } : null;
}
...
function mergeBooking(existing: ExtractedAccountBooking, incoming: ExtractedAccountBooking) {
  return {
    ...
    currency: incoming.cashTotal > 0 ? incoming.currency : existing.currency,
    guests: existing.guests === 1 ? incoming.guests : existing.guests,
    cashTotal: incoming.cashTotal > 0 ? incoming.cashTotal : existing.cashTotal,
    awardLabel: incoming.awardLabel ?? existing.awardLabel,
    pointsPrice: incoming.pointsPrice ?? existing.pointsPrice,
    priceSource: incoming.cashTotal > 0 ? "cash" : incoming.pointsPrice ? "points" : incoming.awardLabel ? "free_night" : existing.priceSource,
